...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.cors(Customizer.withDefaults());   // 위에서 만든 cors 설정 security 적용
        http.csrf(csrf -> csrf.disable());                  // 서버사이드 렌더링 방식이 아니니 REST API 방식에서는 비활성화
        http.formLogin(formLogin -> formLogin.disable());   // 서버사이드 렌더링(SSR) 로그인 방식 비활성화
...
//            auth.anyRequest().permitAll()   // .permitAll() 모든 인증 요청 X
        ...
        // HttpSecurity 객체에 설정한 모든 정보를 기반으로 build 하여 SecurityFilterChain 객체 생성 후 Bean 등록
        return http.build();
    }
}
